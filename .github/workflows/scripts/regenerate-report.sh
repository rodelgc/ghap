...
set_paths() {
    REPORT_PATH="$REPORT_ROOT/$TEST_WORKFLOW"
    DATA_PATH="$DATA_ROOT/$TEST_WORKFLOW"

    if [[ $TEST_WORKFLOW == "pr" ]]; then
        REPORT_PATH="$REPORT_PATH/$PR_NUMBER/$TEST_TYPE"
        DATA_PATH="$DATA_PATH/$PR_NUMBER/$TEST_TYPE"
    else
        REPORT_PATH="$REPORT_PATH/$TEST_TYPE"
        DATA_PATH="$DATA_PATH/$TEST_TYPE"
    fi
}
